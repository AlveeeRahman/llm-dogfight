//! The battle loop: pace frames, watch for lag, exit on any key.
...
pub struct RunOpts {
    ...
    /// `--shot SECS:FILE`: write the live frame at that second as a full-frame .ansi snapshot
    pub shot: Option<(f32, String)>,
}
...
pub fn run(cfg: &Config, opts: RunOpts) -> i32 {
    ...
    let name = opts.scene.clone().unwrap_or_else(|| "ufo".into());
    let (cols, rows) = term.size();
    let mut cv = Canvas::new(cols, rows);
    let mut enc = Encoder::new(cfg.truecolor(), cfg.tolerance);
    let mut cur: Box<dyn Scene> = match scenes::make(&name, seed, cv.w, cv.h, cfg, true) {
        Some(s) => s,
        None => {
            term.leave();
            eprintln!("dogfight: unknown scene '{name}' (ufo or ufo-battle)");
            return 2;
        }
    };
    ...
    loop {
        ...
        cur.render(&mut cv);
        if let Some((at, path)) = &opts.shot {
            if !shot_done && started.elapsed().as_secs_f32() >= *at {
